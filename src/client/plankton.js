...
export default class Plankton {
  constructor(scene, player) {
    ...
    this.plankton = [];
    this.fishData = null;
    this.density = 0.05; // Default values until fetched
  ...
  init() {
    if (!this.fishData) {
      console.warn("fishData not loaded yet, delaying plankton init");
      setTimeout(() => this.init(), 100); // Retry after a short delay
      return;
    }
  ...
  update(delta) {
    this.plankton.forEach((plankton) => {
      plankton.rotation.y += plankton.rotationSpeed;
      ...
      if (plankton.moveTimer <= 0 && !plankton.isMoving) {
        ...
        plankton.userData.velocity = direction.multiplyScalar(2);
      }
      if (plankton.isMoving) {
        plankton.position.addScaledVector(
          plankton.userData.velocity,
          delta * 60
        );
        plankton.moveTimer -= delta;
